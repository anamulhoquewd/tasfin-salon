...
async function seed() {
  ...
  // Super admin
  const existing = await User.findOne({ email: 'admin@tasfin.com.bd' });
  if (!existing) {
    const hashed = await bcrypt.hash('tasfin2026', 10);
    await User.create({ email: 'admin@tasfin.com.bd', password: hashed, name: 'Naima R.', role: 'super_admin' });
    console.log('Created super admin: admin@tasfin.com.bd / tasfin2026');
  } else {
  ...
  if (reelCount === 0) {
    await ReelSettings.create({ src: '', autoplay: true, maxLoops: 2 });
  ...
  if (serviceCount === 0) {
    await Service.insertMany([
      ...
      { name: 'Hair Color', category: 'Hair', icon: 'palette', duration: '2 hr – 4 hr', price: '৳ 6,500 – 14,500', desc: 'Single tone, balayage, or correction. We patch-test and book a follow-up gloss four weeks out.', order: 4 },
      { name: 'Signature Facial', category: 'Facial', icon: 'leaf', duration: '1 hr 15 min', price: '৳ 4,800', desc: 'Cleanse, exfoliate, mask, and massage with cosmeceutical-grade products. Calming, not aggressive.', order: 5 },
      { name: 'Hydra-glow', category: 'Facial', icon: 'droplet', duration: '1 hr', price: '৳ 6,200', desc: 'Hydradermabrasion for clear, dewy skin before an event or shoot. Zero downtime.', order: 6 },
      { name: 'Full-body Wax', category: 'Wax', icon: 'flame', duration: '1 hr 30 min', price: '৳ 3,800', desc: 'Warm wax, fast and gentle. Includes underarm and bikini line.', order: 7 },
      { name: 'Mani-pedi', category: 'Nail', icon: 'hand', duration: '1 hr 15 min', price: '৳ 2,800', desc: 'Soak, shape, cuticle care, and a long-wear gel finish in your choice of palette.', order: 8 },
      { name: 'Nail Art', category: 'Nail', icon: 'sparkles', duration: '45 min', price: '৳ 1,800', desc: 'Hand-painted designs, French tips, or chrome — added on to any mani.', order: 9 },
  ...
  if (testCount === 0) {
    await Testimonial.insertMany([
      { name: 'Naima R.', role: "Bride · Dec 2025", quote: "I've never had makeup last twelve hours like this. The trial alone was worth it — they actually listened and didn't redo my face into someone else's.", order: 1 },
      { name: 'Tahsin A.', role: 'Regular client', quote: "First salon in Dhaka where the haircut booking was on time, the wash was unhurried, and the cut looked the same a month later.", order: 2 },
      { name: 'Sumaiya K.', role: "Bride · Sept 2025", quote: "Brought my mother and aunt for the engagement and they're both still talking about it. Calm, well-run, and not at all rushed.", order: 3 },
    ]);
  ...
  if (faqCount === 0) {
    await Faq.insertMany([
      { question: 'Do I need to book in advance?', answer: 'Bridal services book out 3–6 months ahead. For everything else, WhatsApp us — we usually have weekday afternoon slots within the week.', order: 1 },
      ...
      { question: 'Is there parking?', answer: 'Two spots in front of the building, plus on-street on Road 11 after 6 PM. The Gulshan 2 circle is a 4-minute walk.', order: 4 },
    ]);
  ...
  if (bookingCount === 0) {
    ...
    const channels: Array<'WhatsApp' | 'Form' | 'Walk-in'> = ['WhatsApp', 'WhatsApp', 'WhatsApp', 'Form', 'Walk-in'];
    const statuses: Array<'confirmed' | 'pending'> = ['confirmed', 'confirmed', 'confirmed', 'pending'];
    const serviceNames = ['Bridal Makeup', 'Hair Color', 'Signature Facial', 'Mani-pedi', 'Threading', 'Haircut & Style', 'Engagement Look'];
    const clientNames = ['Naima Rahman', 'Tahsin Akter', 'Sumaiya Khan', 'Farzana B.', 'Nadia H.', 'Ayesha M.', 'Reema K.', 'Nusrat J.', 'Lubna R.'];
    ...
    sampleBookings.push({
      ...
      channel: 'WhatsApp',
      price: '৳ 18,500',
      rawMessage: 'Assalamu alaikum! Ami May 9 e bridal makeup korate chai. Time 11 AM. Dhonnobad.',
      parseConfidence: 0.94,
      autoReply: 'Walaikum assalam Naima Apu! 9 May, 11 AM e Bridal Makeup confirm korte parchi.',
    ...
    sampleBookings.push({
      ...
      channel: 'WhatsApp',
      price: '৳ 6,500',
      rawMessage: 'Hi, hair color appointment chai Wednesday afternoon. Around 4-5 pm hole valo. Balayage, medium length.',
      parseConfidence: 0.88,
      autoReply: 'Hi Tahsin! 6 May Wednesday 4:30 PM e Balayage er jonno slot ache. Confirm korbo?',
    ...
    sampleBookings.push({
      ...
      channel: 'Form',
      price: '৳ 5,000',
      rawMessage: 'Form submission · service: Bridal Trial · preferred: Tue 5 May, 12:00 PM',
      parseConfidence: 1.0,
